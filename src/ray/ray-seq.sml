(* Ray-tracer written by Troels Henriksen, DIKU *)
...
(* Modified by Martin Elsman:

 0. Make use of real-blocks to avoid boxing of reals.

 1. Make a number of other MLKit-friendly transformations for
    generating efficient code.

 2. Disable all parallelisation.
*)
...
fun black () : colour = V.mk{x=0.0, y=0.0, z=0.0}
fun white () : colour = V.mk{x=1.0, y=1.0, z=1.0}
...
fun aabb_hit (aabb:aabb_packed) ({origin, dir}: ray) tmin0 tmax0 =
  let fun iter min' max' origin' dir' tmin' tmax' =
          ...
          in (if invD < 0.0 then max t1 tmin' else max t0 tmin',
              if invD < 0.0 then min t0 tmax' else min t1 tmax')
          end
...
val G = CommandLineArgs.parseInt "G" 1600
...
(*                val () = print "Calling update\n"*)
            in Array.update (pixels,
                             l,
                             colour_to_pixel (trace_ray objs width height cam j i)) end
(*        val () = print "Calling parfor\n" *)
        val _ = ForkJoin.parfor G (0,height*width) pixel
    in {width = width,
        height = height,
        pixels = pixels
       }
...
val () = print ("Spheres(rgbbox) = " ^ Int.toString (length (#spheres rgbbox)) ^ "\n")
...
val height = CommandLineArgs.parseInt "m" 768
val width = CommandLineArgs.parseInt "n" 1024
...
(*val endTiming = Timing.start "Scene BVH construction"*)
