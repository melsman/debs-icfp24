...
functor DLXSimulatorFun (structure RF : REGISTERFILE
  ...
  = struct
	...
	| ILLEGAL;
      ...
      (* Imperative input stream *)
      local
        val inputs : int list ref = ref []
      in
        fun inputsSet l = inputs := l
        fun inputsGet1 () : int option =
            case inputs of
                ref (x::rest) => (inputs := rest; SOME x)
              | _ => NONE
      end
	...
	| LoadProgAux (instrs::instr_list, mem, address)
       ...
       * initialised memory.
       *)
      fun run_prog instructions args
	  = ( inputsSet args
