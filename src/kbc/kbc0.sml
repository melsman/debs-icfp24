...
fun partition p l =
    let fun part_rec []   = ([],[])
          | part_rec (a::L) =
            let val (pos,neg) = part_rec L
            in if p a then  ((a::pos), neg) else (pos, (a::neg))
            end
    in part_rec l
    end
...
fun print_num n = print(Int.toString n)
...
fun matching term1 term2 =
  let fun match_rec subst (Var {n=v}, M) =
...
fun copy_rules [] = []
  | copy_rules ((k,(n,(M,N)))::rest) = (k,(n,(copy_term M, copy_term N))):: copy_rules rest
...
fun rename n (t1,t2) =
    let fun ren_rec (Var {n=k}) = var(k+n)
...
fun kb_completion (arg as (n, rules, list, k, l, eps)) =
    let fun kbrec count n rules failures k l eqs  =
            let fun normal_form x = mrewrite_all rules x
                fun get_rule k = assoc k rules
                fun process failures k l eqs =
                    let fun processf k l eqs =
                            let fun processkl [] =
                                  ...
                                  | processkl ((M,N)::eqs) =
                                    let val M' = normal_form M
                                        ...
                                        fun enter_rule (left,right) =
                                            let val new_rule = (n+1, mk_rule left right)
...
fun main (name:string,args:string list) =
    let fun doit () = kb_complete  [] (* terms in list global *) (Geom_rules())

        fun loop n = if n <= 1 then doit()
                     else (doit(); loop (n-1))
    in
      print ("Completed rules: " ^ Int.toString (loop 20) ^ "\n");
